...
it('creates a derived atom using the same atom using selectors', () => {
  const $firstAtom = atom({ alpha: 3, beta: 5 })
  const $atom = atom((get) => get($firstAtom.focus('alpha')) + get($firstAtom.focus('beta')))
  expect(debug($atom)).toMatchSnapshot()
})
...
test('can derive state from external sources', () => {
  const $fakeReduxBase = atom(8)
  const fakeRedux = {
    getState: () => $fakeReduxBase.value,
    subscribe: (item: any) => $fakeReduxBase.subscribe(item),
  }

  const $reduxAtom = atom((get) => get(fakeRedux.getState, fakeRedux.subscribe))

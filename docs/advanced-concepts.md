...
With an additional overload of the `get` function, you can consume external (non-**xoid**) sources. This can be a Redux store, an RxJS observable, or anything that implements a `getState` & `subscribe` pair. Here is an atom that derives its state from a Redux store:
...
An enhanced atom is an atom whose default `.set` method is swapped with something else. This technique can be used to create "pass through atoms" that act as mediators. Most people using **xoid** will not need to write enhanced atoms. 

...
var select = function (atom, selector) {
    ...
    var relativePath = isPluck ? [selector] : fn(addressProxy([]))[engine.RECORD];
    var meta = atom[engine.META];
    var path = (atom[engine.RECORD] || []).concat(relativePath);
    ...
    var target = engine.createTarget(getValue, setValue);
    target[engine.META] = meta;
    target[engine.RECORD] = path;
    if (!hasSymbol) {
...
function create(init, usable, enhancer) {
    ...
    if (typeof init === 'function') {
        ...
        isValid = false;
        var _a = engine.createCleanup(), onCleanup = _a.onCleanup, cleanupAll_1 = _a.cleanupAll;
        var getter_1 = engine.createGetState(function () {

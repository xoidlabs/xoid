...
import { Store, Abstract, GetStoreState } from '../core/types'
import { storeMap, useIsoLayoutEffect, destroy } from '../core/utils'

export function useStore<G extends Store<any, any> | Abstract<any>>(
  store: G | (() => G)
): G extends Store<infer T, infer A>
  ? [GetStoreState<G>, A]
  : G extends Abstract<infer T>
  ...
  const storeRef = ref.current as Abstract<any>
